...
%
function [hdr, filetype, fileprefix, machine, filename, compressed] = load_nii_hdr(fileprefix)
   ...
   if findstr('.gz', fileprefix)
       oldfilename = fileprefix;
       compressed = 1;
       fileprefix = char( gunzip(oldfilename) );
       filename = fileprefix;
   end
      ...
      if fread(fid,1,'int32') == 348
         ...
         if fid < 0,
            ...
            if fread(fid,1,'int32') ~= 348
               ...
               %  Now throw an error
               %
               msg = sprintf('File "%s" is corrupted.',filename);
               error(msg);
            end
