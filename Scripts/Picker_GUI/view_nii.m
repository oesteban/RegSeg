...
function status = view_nii(varargin)
   ...
   case {'axial_image','coronal_image','sagittal_image'}    
      ...
      elseif strcmpi(click,'alt')&&isfield(nii_view.handles,'vox_select')
...
%           vox_stats = get(nii_view.handles.vox_stats,'Data');
          ...
          mask(sag-1:sag+1,cor-1:cor+1,axi-1:axi+1) = conn;

          VoxIntensity = getappdata(fig,'VoxIntensity');
          VoxIntensity = add_new_voxel(nii_view, VoxIntensity, img, mask, imgvalue);
%           roi = [];
          % Case Gray Matter; first row of VoxIntensity
%           if (VoxIntensity{1,1}<npoints) && get(nii_view.handles.grayM,'Value')
%               k = 1; while ~isempty(points{k,1})&&(k<size(points,1)), k = k+1; end
%               points{k,1} = imgvalue;
%               roi = 'Gray Matter';
%               for iScan = 1:(size(VoxIntensity,2)-1)
%                   img_neighbour = mask.*double(squeeze(img(:,:,:,nii_view.scanid+(iScan-1))));
%                   imgpoints = img_neighbour(mask~=0);
%                   VoxIntensity{1,iScan+1} = [VoxIntensity{1,iScan+1}; imgpoints];
%                   vox_stats{iScan, 3}  = mean(VoxIntensity{1,iScan+1});
%                   vox_stats{iScan, 4} = std(VoxIntensity{1,iScan+1});
%               end
% %               if VoxIntensity{1,1}<(npoints-1), 
%               VoxIntensity{1,1} = VoxIntensity{1,1}+1;
% %               else VoxIntensity{1,1} = roi; 
% %               end
%               
%           % Case White Matter; second row of VoxIntensity
%           elseif VoxIntensity{2,1}<npoints && get(nii_view.handles.whiteM,'Value')
% 
%               k = 1; while ~isempty(points{k,2})&&(k<size(points,1)), k = k+1; end
%               points{k,2} = imgvalue;
%               roi = 'White Matter';
%               for iScan = 1:(size(VoxIntensity,2)-1)
%                   img_neighbour = mask.*double(squeeze(img(:,:,:,nii_view.scanid+(iScan-1))));
%                   imgpoints = img_neighbour(mask~=0);
%                   VoxIntensity{2,iScan+1} = [VoxIntensity{2,iScan+1}; imgpoints];
%                   vox_mean{iScan,2}  = mean(VoxIntensity{2,iScan+1});
%                   vox_stddev{iScan,2} = std(VoxIntensity{2,iScan+1});
%               end
% %               if VoxIntensity{2,1}<(npoints-1), 
%               VoxIntensity{2,1} = VoxIntensity{2,1}+1;
% %               else VoxIntensity{2,1} = roi; 
% %               end
%               
%           % Case CSF; third row of VoxIntensity     
%           elseif VoxIntensity{3,1}<npoints && get(nii_view.handles.csf,'Value')
%               
%               k = 1; while ~isempty(points{k,3})&&(k<size(points,1)), k = k+1; end
%               points{k,3} = imgvalue;              
%               roi = 'CSF';
%               for iScan = 1:(size(VoxIntensity,2)-1)
%                   img_neighbour = mask.*double(squeeze(img(:,:,:,nii_view.scanid+(iScan-1))));
%                   imgpoints = img_neighbour(mask~=0);
%                   VoxIntensity{3,iScan+1} = [VoxIntensity{3,iScan+1}; imgpoints];
%                   vox_mean{iScan,3} = mean(VoxIntensity{3,iScan+1});
%                   vox_stddev{iScan,3} = std(VoxIntensity{3,iScan+1});
%               end
% %               if VoxIntensity{3,1}<(npoints-1), 
%               VoxIntensity{3,1} = VoxIntensity{3,1}+1;
% %               else VoxIntensity{3,1} = roi; 
% %               end
%               
%           end
