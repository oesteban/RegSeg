...
def evaluation_workflow( name="Evaluation",
    ...
    noise = pe.Node( misc.AddNoise(snr=20), name='AddNoise' )
    ...
    trk_gold = dmri.mrtrix_tractography_workflow(name='Tract_Gold', seed='gm' )
...
if __name__ == '__main__':
    ...
    evaluation = evaluation_workflow( name='Evaluation2', direction='z' )
    ...
    in_dwi = op.join( model_dir, 'hardi', 'DWIS_hardi-scheme.nii.gz' )
    ...
    in_bvec = op.join( model_dir, 'hardi', 'hardi-scheme.bvec' ) 
    in_bval = op.join( model_dir, 'hardi', 'hardi-scheme.bval' ) 
    ...
    evaluation.inputs.inputnode.in_bval = in_bval
    evaluation.run()
