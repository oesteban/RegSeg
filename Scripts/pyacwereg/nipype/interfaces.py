...
# @Last Modified time: 2014-10-14 19:39:19
...
class ACWERegInputSpec(ACWERegInputGroupSpec):
    ...
    images_verbosity = traits.Int(
        1, desc='verbosity of intermediate results output', argstr='-v %d')


class ACWERegOutputSpec(TraitedSpec):
    out_warped = OutputMultiPath(
        File(exists=True, desc='source images unwarped'))
    out_surfs = OutputMultiPath(
        File(exists=True, desc='priors in target space'))
    out_tpms = OutputMultiPath(
        File(exists=True, desc='tissue probability maps (TPM) in target space'))
...
class RandomBSplineDeformation(ANTSCommand):
    ...
    def _list_outputs(self):
        ...
        outputs['out_coeff'] = [
            op.abspath('%s_coeffs_%d.nii.gz' % (out_prefix, i)) for i in range(3)]
        outputs['out_field_base'] = [
            op.abspath('%s_field_cmp%d.nii.gz' % (out_prefix, i)) for i in range(3)]

        if isdefined(self.inputs.in_surfs):
            outputs['out_surfs'] = [op.abspath(
                '%s_warped_%d.vtk' % (out_prefix, i)) for i in range(len(self.inputs.in_surfs))]
    ...
    def _list_outputs(self):
        ...
        if isdefined(self.inputs.in_surf):
            outputs['out_surf'] = [op.abspath(
                '%s_warped_%d.vtk' % (out_prefix, i)) for i in range(len(self.inputs.in_surf))]
        return outputs
