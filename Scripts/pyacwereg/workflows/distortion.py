...
# @Last Modified time: 2014-10-14 19:14:28
...
from nipype.interfaces import ants as ants           # ANTS
...
def isbi_workflow(name='ISBI2014'):
    ...
    inputnode = pe.Node(niu.IdentityInterface(
                        fields=['subject_id', 'in_fmap_mag', 'in_fmap_pha',
                                'in_t1w', 'in_t2w', 'fs_subjects_dir',
                                'te_incr', 'echospacing', 'enc_dir']),
                        name='inputnode')
    outputnode = pe.Node(niu.IdentityInterface(
                         fields=['out_file', 'out_vsm',
                                 'out_mask', 'out_tpms']),
                         name='outputnode')
    ...
    workflow.connect([
        (inputnode,  prepare, [
            ...
            ('in_fmap_mag', 'inputnode.in_fmap_mag'),
            ('in_fmap_pha', 'inputnode.in_fmap_pha'),
            ('in_t1w', 'inputnode.in_t1w'),
            ('in_t2w', 'inputnode.in_t2w'),
            ('fs_subjects_dir', 'inputnode.fs_subjects_dir')]),
        (inputnode,  distort, [('te_incr', 'inputnode.te_incr'),
                               ('echospacing', 'inputnode.echospacing'),
                               ('enc_dir', 'inputnode.enc_dir')]),
        (prepare,    distort, [('outputnode.out_smri', 'inputnode.in_file'),
                               ('outputnode.out_fmap_pha', 'inputnode.in_pha'),
                               ('outputnode.out_fmap_mag', 'inputnode.in_mag'),
                               ('outputnode.out_mask', 'inputnode.in_mask'),
                               ('outputnode.out_tpms', 'inputnode.in_tpms')]),
        (distort, outputnode, [('outputnode.out_file', 'out_file'),
                               ('outputnode.out_vsm', 'out_vsm'),
...
def bspline_deform(name='BSplineDistortion', n_tissues=3):
    ...
    inputnode = pe.Node(niu.IdentityInterface(
                        fsields=['in_file', 'in_tpms', 'in_mask', 'in_surfs',
                                 'grid_size']),
                        name='inputnode')
    outputnode = pe.Node(niu.IdentityInterface(
                         fields=['out_file', 'out_tpms', 'out_surfs',
    ...
    norm_tpms = pe.Node(Normalize(), name='NormalizeTPMs')
