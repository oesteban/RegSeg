...
def t2_registration_correct(name="T2_Registration"):
    ...
    outputnode = pe.Node(niu.IdentityInterface(
                         fields=['epi_corrected', 'out_mask', 'fwd_tfm']),
                         name='outputnode')
...
#    reg.inputs.collapse_output_transforms = False
    ...
    applytfm = pe.Node(
        ants.WarpTimeSeriesImageMultiTransform(), name='ApplyTfm')
    ...
    applytfm_msk = pe.Node(
        ants.WarpImageMultiTransform(use_nearest=True), name='ApplyTfmMsk')

    pipeline.connect([
        (inputnode,      get_b0, [('in_file', 'in_file')]),
        (inputnode,    resample, [('in_t2', 'in_file'), ]),
        (get_b0,       resample, [
            (('roi_file', get_vox_size), 'vox_size')]),
        ...
        (reg,          applytfm, [
         ('forward_transforms', 'transformation_series')]),
...
def distortion_workflow(name="synthetic_distortion"):
    ...
    inputnode = pe.Node(niu.IdentityInterface(fields=[
                        'in_file', 'in_mask', 'te_incr', 'echospacing', 'encoding_direction', 'intensity', 'sigma', 'in_tpms']), name='inputnode')
    phmap_siemens = pe.Node(niu.Function(input_names=['in_file', 'intensity', 'sigma'], output_names=[
                            'out_file'], function=generate_siemens_phmap), name='gen_siemens_PhaseDiffMap')
    prepare = pe.Node(
        fsl.PrepareFieldmap(nocheck=True), name='fsl_prepare_fieldmap')
    vsm = pe.Node(fsl.FUGUE(save_shift=True), name="gen_VSM")
    dm = pe.Node(niu.Function(input_names=['in_file', 'in_mask'], output_names=[
                 'out_file'], function=demean), name='demean')
    applyWarp = fugue_all_workflow()
    vsm_fwd_mask = pe.Node(
        fsl.FUGUE(forward_warping=True), name='Fugue_WarpMask')
    binarize = pe.Node(fs.Binarize(min=0.00001), name="binarize")
    warpimages = pe.MapNode(fsl.FUGUE(
        forward_warping=True, icorr=True), iterfield=['in_file'], name='WarpImages')
    normalize_tpms = pe.Node(niu.Function(input_names=['in_files'], output_names=[
                             'out_files'], function=normalize), name='Normalize')
#    fixsignal = pe.Node( niu.Function( input_names=['in_reference','in_distorted','in_mask' ], output_names=['out_distorted'], function=sanitize ), name='FixSignal' )
    ...
    outputnode = pe.Node(niu.IdentityInterface(
                         fields=['out_file', 'out_vsm', 'out_mask', 'out_phdiff_map', 'out_tpms']), name='outputnode')
    ...
    pipeline.connect([
        ...
        (inputnode,      prepare, [('in_mask', 'in_magnitude'),
                                   (('te_incr', _sec2ms), 'delta_TE')]),
        (prepare,            vsm, [
            ('out_fieldmap', 'phasemap_file')]),
        (inputnode,          vsm, [('in_mask', 'in_file'),
                                   ('in_mask', 'mask_file'),
                                   ('te_incr', 'asym_se_time'),
        ...
        (inputnode,           dm, [('in_mask', 'in_mask')]), (inputnode, vsm_fwd_mask, [('in_mask', 'in_file'), ('in_mask', 'mask_file')]), (dm,        vsm_fwd_mask, [('out_file', 'shift_in_file')]), (dm,           applyWarp, [('out_file', 'inputnode.in_vsm')]), (inputnode,    applyWarp, [('in_file', 'inputnode.in_file'), ('in_mask', 'inputnode.in_mask'), ('encoding_direction', 'inputnode.unwarp_direction')]), (dm,          outputnode, [('out_file', 'out_vsm')]), (applyWarp,   outputnode, [
            ('outputnode.out_file', 'out_file')]), (vsm_fwd_mask,  binarize, [('warped_file', 'in_file')]), (binarize,    outputnode, [('binary_file', 'out_mask')]), (phmap_siemens, outputnode, [('out_file', 'out_phdiff_map')]), (inputnode,  warpimages, [('in_tpms', 'in_file'), ('in_mask', 'mask_file')]), (dm,   warpimages, [('out_file', 'shift_in_file')]), (warpimages,  normalize_tpms, [('warped_file', 'in_files')]), (normalize_tpms,  outputnode, [('out_files', 'out_tpms')])
    ])
...
def mrtrix_tractography_workflow(name='MRTrix_CSD_tracks', thres=0.5, seed='wm'):
    ...
    csdeconv = pe.Node(
        mrt.ConstrainedSphericalDeconvolution(), name='csdeconv')
    csdeconv.inputs.maximum_harmonic_order = 6
    track = pe.Node(
        mrt.SphericallyDeconvolutedStreamlineTrack(), name='CSDstreamtrack')
    ...
    pipeline.connect([
        (inputnode,    fsl2mrtrix, [("in_bvec", "bvec_file"), ('in_bval', 'bval_file')]), (inputnode,        wmmask, [('in_mask', 'in_file')]), (inputnode,      est_resp, [("in_dwi", "in_file")]), (fsl2mrtrix,     est_resp, [("encoding_file", "encoding_file")]), (wmmask,         est_resp, [("binary_file", "mask_image")]), (inputnode,      csdeconv, [("in_dwi", "in_file")]), (wmmask,         csdeconv, [("binary_file", "mask_image")]), (est_resp,       csdeconv, [("response", "response_file")]), (fsl2mrtrix,     csdeconv, [
            ("encoding_file", "encoding_file")]), (csdeconv,          track, [("spherical_harmonics_image", "in_file")]), (track,           tck2trk, [('tracked', 'in_file')]), (inputnode,       tck2trk, [('in_dwi', 'image_file')]), (tck2trk,      outputnode, [('out_file', 'track_file')]), (tck2trk,          matrix, [('out_file', 'in_file')]), (inputnode,        matrix, [('roi_file', 'roi_file')]), (matrix,       outputnode, [('out_conmat', 'connectome'), ('out_lenmat', 'avglen_map'), ('out_tracks', 'matrix_tracks')])
    ])

    if seed == 'wm':
        pipeline.connect(
            [(wmmask,          track, [("binary_file", "seed_file")])])
    else:
        seedbin = pe.Node(fs.Binarize(min=thres), name='BinSM')
        pipeline.connect([
            (inputnode,    seedbin, [('roi_file', 'in_file')]), (seedbin,        track, [
                ("binary_file", "seed_file")])
        ])
...
def dtk_tractography_workflow(name='DTK_tractography', seed='wm'):
    ...
    dtk_tracker = pe.Node(dtk.DTITracker(mask1_threshold=[
                          0.002, 3.0], invert_x=False, invert_y=True, random_seed=10), name="dtk_tracker")
    ...
    pipeline.connect([
        (inputnode,        dtifit, [('in_dwi', 'DWI'), ('in_bvec', 'bvecs'), ('in_bval', 'bvals')]), (inputnode,   dtk_tracker, [('in_mask', 'mask1_file')]), (dtifit,      dtk_tracker, [('tensor', 'tensor_file')]), (dtk_tracker,  smooth_trk, [('track_file', 'track_file')]), (dtifit,       outputnode, [('tensor', 'tensor'), ('ADC', 'ADC'), ('FA', 'FA')]), (dtk_tracker,  outputnode, [
            ('track_file', 'track_file')]), (smooth_trk,   outputnode, [('smoothed_track_file', 'smoothed_track_file')]), (smooth_trk,       matrix, [('smoothed_track_file', 'in_file')]), (inputnode,        matrix, [('roi_file', 'roi_file')]), (matrix,       outputnode, [('out_conmat', 'connectome'), ('out_lenmat', 'avglen_map'), ('out_tracks', 'filter_tracks')])
...
def fugue_all_workflow(name="Fugue_WarpDWIs"):
    ...
    inputnode = pe.Node(niu.IdentityInterface(
        fields=['in_file', 'in_mask', 'in_vsm', 'unwarp_direction']), name='inputnode')
    dwi_split = pe.Node(niu.Function(input_names=['in_file'], output_names=[
                        'out_files'], function=_split_dwi), name='split_DWI')
    vsm_fwd = pe.MapNode(fsl.FUGUE(forward_warping=True, icorr=False), iterfield=[
                         'in_file'], name='Fugue_Warp')
    dwi_merge = pe.Node(fsl.utils.Merge(dimension='t'), name='merge_DWI')

    outputnode = pe.Node(
        niu.IdentityInterface(fields=['out_file']), name='outputnode')
    pipeline.connect([
        (inputnode,  dwi_split, [('in_file', 'in_file')]), (dwi_split,    vsm_fwd, [('out_files', 'in_file')]), (inputnode,    vsm_fwd, [
            ('in_mask', 'mask_file'), ('in_vsm', 'shift_in_file'), ('unwarp_direction', 'unwarp_direction')]), (vsm_fwd,    dwi_merge, [('warped_file', 'in_files')]), (dwi_merge, outputnode, [('merged_file', 'out_file')])
    ])
...
def normalize(in_files):
    ...
    nib.save(nib.Nifti1Image(
        weights, im.get_affine(), im.get_header()), op.abspath('w.nii.gz'))
...
def sanitize(in_reference, in_distorted, in_mask, out_file=None):
    ...
    for i in range(0, np.shape(im_data)[-1]):
        im_data[:, :, :, i] = ref_data[:, :, :, i] * \
            msk_data + im_data[:, :, :, i] * (1 - msk_data)
    ...
    nib.save(
        nib.Nifti1Image(im_data, im.get_affine(), im.get_header()), out_file)
    return out_file
...
def generate_siemens_phmap(in_file, intensity, sigma, w1=-0.5, w2=1.0, out_file=None):
   ...
   # phasefield= phasefield * normalizer + noisesrc
    ...
    nib.save(nib.Nifti1Image(
        phasefield.astype(np.float32), msk.get_affine(), hdr), out_file)
    return out_file
