...
class FSFilesInputSpect(BaseInterfaceInputSpec):
    ...
    subject_id = traits.String(
        mandatory=True, desc='Subject in Freesurfer system')
...
class FSFiles(IOBase):
    ...
    def _list_outputs(self):
        fs_dir = op.join(
            self.inputs.fs_subjects_dir, self.inputs.subject_id, 'mri')
...
def gen_fs_transform(in_file, fs_subjects_dir, subject_id, out_file=None):
    ...
    sp.check_call(["tkregister2", "--mov %s" % in_file, "--targ %s" %
                   in_orig, "--reg %s" % out_file, "--noedit", "--regheader"])
    return out_file
...
def transform_surface(in_surf, in_reg, in_target, fs_subjects_dir, subject_id, out_file=None):
    ...
    try:
        cmd = "SUBJECTS_DIR='%s'; mri_surf2surf --sval-xyz %s --reg %s %s --tval %s --tval-xyz --hemi %s --s %s" % (
            fs_subjects_dir, surf, in_reg, in_target, out_file, hemi, subject_id)
...
def merge_labels(in_file, labels):
    ...
    nb.save(
        nb.Nifti1Image(outdata, img.get_affine(), img.get_header()), out_file)
    return out_file
