...
def transformVolume( mov_path, tar_path, tfmfile, invert=False, outname='' ):
    ...
    target_to_tkr = np.dot( tar_vox2ras_tkr, np.linalg.inv( nib.load( tar_path ).get_affine() ) )
    ...
    if not invert:
        result = resample( movimg, tarimg.coordmap, np.linalg.inv( xfm ) , np.shape(tarimg.get_data()) )
        if outname=='':
            outname= op.join( op.dirname( mov_path), '%s-to-%s.nii.gz' % ( getfname( mov_path ), getfname( tar_path ) ) )
    else:
        result = resample( tarimg, movimg.coordmap, xfm, np.shape(movimg.get_data()) )
        if outname=='':
            outname= op.join( op.dirname( tar_path), '%s-to-%s.nii.gz' % ( getfname( tar_path ), getfname( mov_path ) ) )
...
def transformSurface(origin, mov_path, tar_path, tfmfile, out_file="", use_paraview=False, write_vtp=False, del_vtk=False ):
    ...
    if use_paraview:
        ...
        phy_size = np.dot( ac[0:3,0:3], np.array(nib.load(tar_path).get_data().shape) )
        #print phy_size
        #M[0,0:3]*=-1
        #M[1,1]*=-1
        O[0]-=phy_size[0]*0.5
