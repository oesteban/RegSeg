...
of a certain image $X$ in piecewise smooth regions $\Omega_k$, 
with $k \in \lbrace 1 .. K \rbrace$, that maximizes the a posteriori 
...
disjoint region set $\Omega_k$. $p(X \mid Y)$ is the \emph{likelihood} of 
the realization of $X$ (the image) given a certain distribution model for 
each region $k$. The second term, $p(Y)$, is the a-priori probability of 
