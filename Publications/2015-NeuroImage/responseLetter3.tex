...
We have added this clarification in lines 361--364, in the
  discussion section.
}
\begin{reviewpoint}
...
\item Better positioned labels in the panel A of this figure, using the Fruchterman-Reingold force-directed algorithm.
  This functionality has been included in the code release of the paper.
...
\end{itemize}
Additionally, we have included in the Supplemental Materials (new figures S8 and S9) zoomed versions of all the joint plots for
...
How this methodological variation would affect posterior processing (analyzing tracks, interpolating FA on them, building the connectivity matrix, etc.) is discussed in the paragraph containing lines 359--360.
Particularly, these lines 359--360 were inserted to address this comment, which is highly related to Comments \ref{com:1} (Reviewer \#1), \ref{com:5} (R\#3) and \ref{com:6} (R\#3).
}
\newpage
\begin{reviewpoint}
...
\end{reviewpoint}
\reply{%
We have introduced the definition of $\mathbf{g}_k$ in lines 122--125, introducing the former Eq. (A.8) defining the
gradients in the text as new Equation (11).
Also, a clarification has been added in the appendix itself (lines 443--445).
}
...
\end{reviewpoint}
\reply{%
To strongly support the focus of the paper defined in lines 42--43, we have explicitly stated this in a new sentence (lines 356--358).
Additionally, the edits in lines 57--59 introduced in response to \autoref{com:2} also contribute to clarify the primary goal of
  image registration.
}
\begin{reviewpoint}
...
\end{reviewpoint}
\reply{%
Right after the previous edition, in lines 358--359 we introduce the proposal of this methodological variation.
The discussion and proposal of a future research line is then done in lines 364--365.
Some other details are added to cover Comments \ref{com:1} and \ref{com:2} (R\#1) as well.
}
\begin{reviewpoint}
...
\end{reviewpoint}
\reply{%
As described in \autoref{com:7} (R\#3), yes, the coordinate system in which the surfaces are
...
Generally, when we speak of \emph{b0}-to-T2w registration, we identify the direction of
  ...
  T2w-image grid.
However, the underlying transformation works in the opposite way: for each point in
  the reference space (T2w) computes the corresponding point in the moving image (\emph{b0}).
...
New text (lines 227--232) in response to \autoref{com:7} was also included to clarify this point.
}
\newpage
\begin{reviewpoint}
...
\end{reviewpoint}
\reply{%
We have introduced this description in lines 184--188, along with including the detail of the actual HCP release of the data used in line 178.
}
\begin{reviewpoint}
Minor points:
...
We also have fixed the misspelling of ``preliminar'' of line 351.
}
