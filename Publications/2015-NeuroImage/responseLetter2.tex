...
\end{reviewpoint}
\reply{%
The reviewer is totally right. We have modified the text accordingly (lines 117--118,
  and lines 412--414 in the Appendix). 
...
\end{reviewpoint}
\reply{%
We have introduced one Appendix (starting by line 396) explaining how the shape-gradients are applied and how the equation given
...
\end{reviewpoint}
\reply{%
We have introduced the symbol $\mathcal{N}$ before eq. (4) in which it appears for first time (line 90).
}
...
In the revised manuscript, this is described in the new Appendix 1 (equations A.1--4).
}
...
\end{reviewpoint}
\reply{%
Added a sentence describing A and B (lines 103--104).
}
...
\end{reviewpoint}
\reply{%
Added explanation in lines 120--121.
}
...
\end{reviewpoint}
\reply{%
Equations (A.6) and (A.7) introduced in the new Appendix 1 define $\mathbf{g}_k$ (starts by line 406).
Additionally, a reference to the Appendix and corresponding equations (lines 124--125) and a brief
  explanation of $\mathbf{g}_k$ (line 126) have been added.
}
...
We also have included typical ranges for these values in lines 132--133.
}
