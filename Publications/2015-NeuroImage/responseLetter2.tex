...
The fieldmaps are processed to remove the noise present in the air cavities and the background,
  ...
  field.
But they essentially reproduce the original (and real) distortion.
...
This comment is similar to one made by the reviewer \#3 in the \autoref{com:31}.
...
\end{reviewpoint}
\reply{%
We have introduced one Appendix explaining how the shape-gradients are applied and how the equation given
  in the Figure 1 relates to the registration framework.
As the reviewer suggests, this text was part of the Supplemental Materials.
}
...
However, and with certain adaptations, the registration method is also useful in other approaches such those
  suggested.
...
\end{reviewpoint}
\reply{%
We have introduced the name of the model referred to.
}
...
\end{reviewpoint}
\reply{%
We have introduced the symbol $\mathcal{N}$ before eq. (4) in which it appears for first time.
}
...
\end{reviewpoint}
\reply{%
The direction of the normals is explicitly described in the new Appendix 1.
}
...
Again, with the insertion of Appendix 1 we understand that this issue is clarified.
}
...
Additionally, some text has been added to grant the necessary consistency and connect the formulations in
...
\end{reviewpoint}
\reply{%
Added a sentence describing A and B.
}
...
\end{reviewpoint}
\reply{%
We have introduced an intermediate expression to clarify the derivation of the energy functional.
}
...
\end{reviewpoint}
\reply{%
Equations (A.6) and (A.7) introduced in the new Appendix 1 define $\mathbf{g}_k$.
}
...
\end{reviewpoint}
\reply{%
Although it was defined in pg. 2, we have introduced the long form again to make the text more readable.
}
...
\end{reviewpoint}
\reply{%
A bit of text has been added defining $a_i$.
}
...
\end{reviewpoint}
\reply{%
Reference added to that sentence.
}
...
\end{reviewpoint}
\reply{%
In order to ensure the reproducibility of the presented experiments, the actual values
