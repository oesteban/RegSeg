...
Subsequent processing involves describing the local microstructure with one of the available
  ...
  to current models such as \revcomment[R\#3-C.10]{\gls*{amico}}.
...
However, the mask and subsequent analyses are highly dependent on the 
  \revcomment[R\#3-C.33]{threshold that is chosen} \citep{taoka_fractional_2009}.
...
A specific set of reference surfaces extracted from the same subject initialize
  ...
  target-image.
\revcomment[authors]{We apply \regseg{} to segment \gls*{dmri} data by mapping
...
\Regseg{} integrates the benefits of segmentation and registration methods together and
  ...
  1) the surfaces are typically extracted from the \gls*{t1} image of the same subject,
    ...
    \gls*{dmri} protocol \revcomment[R\#1-C.2]{in order to estimate the deformation field};
  2) \revcomment[R\#1-C.2]{depending on the application, the resampling of the \glspl*{dwi}
...
\revcomment[authors]{This approach is the first step of the} \gls*{t2b} correction
  methods \citep{kybic_unwarping_2000}.
\revcomment[authors]{We reproduce the settings and implementation of a} widely used diffusion
