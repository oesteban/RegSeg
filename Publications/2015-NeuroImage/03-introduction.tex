...
Each \gls*{dwi} is sensitized with a gradient to probe proton diffusion in a certain
  \newcomment[RV\#2(C.13)]{orientation}.
...
The microstructural map is then used to draw the preferential \newcomment[RV\#2(C.13)]{orientations} of diffusion
...
\newcomment[RV\#2(C.17)]{Since the proposal of the earliest methodologies to solve the three problems,
  there is a need for precisely identifying the anatomy in the \gls*{dmri} space.
In fact, current trends on reconstruction \citep{jeurissen_multitissue_2014} and
...
\newcomment[RV\#2(C.15)]{The structural information required by pioneer tractography studies was a simple
...
\todo{RV\#2(C.14)}However, the precise segmentation of \gls*{dmri} is difficult for several limitations.
First, \gls{dmri} images have a resolution that is \newcomment[RV\#2(C.12)]{much lower} than that of the imaged
...
Here we show a method that registers \newcomment[RV\#2(C.10)]{anatomically-correct surfaces to multivariate images},
