...
However, both solutions require extra acquisitions to be included within the \gls*{dmri} scanning
  ...
  \gls*{pe} scheme \citep{cordes_geometric_2000,chiou_simple_2000}, \gls*{t2} images
  \citep{kybic_unwarping_2000}.
We will refer to this approach as \gls*{t2b} correction.
...
The tool \emph{bbregister} \citep{greve_accurate_2009} has become a rather standard linear
  registration method to be used with \gls*{epi} corrections.
...
The nonlinear registration approach has been successfully solved with \gls*{t2} and \lowb{}
  images, exploiting their similarity \citep{kybic_unwarping_2000,studholme_accurate_2000}.
...
Here we show a method that simultaneously registers anatomically correct surfaces representing
  the structure of the subject's brain into the bivariate image comprehending \gls*{fa} and
  \gls*{adc}, thereby implicitly segmenting the \gls*{dmri} space.
The hypothesis tested in the present study is that the registration and segmentation
  ...
  correction methods.
Thus, we propose a tool called \regseg{} that 3D active contours (surfaces) without edges
  \citep{chan_active_2001}, which evolve to drive a free-form deformation field of B-spline
  basis functions.
