...
Distortion can be rendered as a registration problem where we look for an underlying deformation 
  field $U_s$, mapping from an undistorted \gls*{t1} image (the reference $R$) to the distorted
  \gls*{dmri} images (the moving space $M$), such that the structures identified in $R$ optimally
  align with the corresponding features in $M$:
...
Joint segmentation and distortion correction of \gls*{dmri} data is also interesting to
  improve the overall consistence of complex processing workflows.
The closest antecedent of \emph{regseg} is \emph{bbregister} \citep{greve_accurate_2009}.
Both methods use active contours to align surfaces to same-subject \gls*{dmri} images.
However, \emph{bbregister} is limited to affine transformations, and thus, it is not able to
  solve the susceptibility-derived distortions, generally nonlinear.
To enable registration of uncorrected data, \emph{bbregister} excludes from the cost-function
  those regions frequently warped.
Conversely, \emph{regseg} implements a free-deformation field, supported with B-Spline
  functions to estimate deformations.
A second diverging foundation is the active-contours framework.
In \emph{regseg}, active contours are implemented without edges \citep{chan_active_2001},
  enabling the tool to segment homogeneous regions within the brain.
On the other hand, \emph{bbregister} requires that the target features present intensity
  boundaries as it implements active contours with edges.
The tissue interfaces imaged with \gls*{dmri} are affected by partial volume effects that
  blur boundaries.
In practice, registration of \gls*{t1} and \gls*{dmri} data with \emph{bbregister}
  is limited to the linear match of the pial surface of the brain.
...
% We demonstrate its aptness on the correction of the nonlinear distortions typically
...
Using the Bayes' rule, the posterior likelihood is computed as:
  ...
  \begin{equation}
  P(U \mid R, \boldsymbol{\Gamma}) = \frac{P(R \mid U,\boldsymbol{\Gamma})\, P(U)}{P(R)},
  \label{eq:bayes_rule}
  \end{equation}
%
  where $P(R \mid U,\boldsymbol{\Gamma})$ is the data-likelihood, and
  $\boldsymbol{\Gamma} = \{\Gamma_{\text{in},\text{out}}\}$ is a set of surfaces 
  corresponding to the interfaces between pairs of competing regions $\Omega_\text{in}$
  and $\Omega_\text{out}$ drawn from the full set of regions $\{\Omega_l \mid l \in \mathbb{N}, l \leq L\}$.
As $\boldsymbol{\Gamma}$ are mapped by $U$, we simplify
  $P(U, \boldsymbol{\Gamma}) = P(U) \implies P(R \mid U,\boldsymbol{\Gamma}) = P(R \mid U)$.

The best estimate $\hat{U}$ then fulfills the maximum a posteriori criterium
  \citep{bishop_pattern_2006}, and aligns $\boldsymbol{\Gamma}$ into $R$.
First, we assume independence between pixels, and thus break down the
  ...
  \begin{equation}
  P(R \mid U) = \underset{l}{\prod} \underset{\vec{r}\in \Omega_l}{\prod}
...
$C$ is the number of channels comprised in the reference image $R$.
