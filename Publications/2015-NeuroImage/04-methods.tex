...
\todo{ordering} Even though the features being segmented are not generally i.i.d., the 
...
The \autoref{fig:regseg-model} shows \revcomment[authors]{how the joint distribution of 
...
\begin{figure}
  ...
  \caption{The {\color{blue} interfacing surfaces $\Gamma_m$ between the competing \glspl{roi} $\Omega_l$,
  play the role of active contours which drive the registration process.}
  They evolve iteratively following {\color{blue} the normal $\hat{\mathbf{n}}_i$ of the surface} at each vertex
  ...
  {\color{blue} The computation of shape-gradients is developed in \ref{app:shape_gradients}.}
...
\vskip-2.0em
\includegraphics[width=\linewidth]{legend}%x
\end{minipage}
...
The smoothness of the resulting \revcomment[R\#3-C.33]{displacement field} is induced
  ...
  \end{align}
 which requires that the distortion and its gradient have zero mean\revcomment[R\#1-C4]{,}
...
Let us assume that \revcomment[R\#1-C.5\\R\#3-C.31]{the preferential directions of
...
\revcomment[R\#3-C.20]{%
  ...
  \end{align}}
  where $\vec{u}^{\circ2} = \vec{u}^T \cdot \vec{u}$, \revcomment[R\#3-C.21]{%
  and $\{\boldsymbol{\alpha}, \boldsymbol{\beta}\}$ are the expected variances along
  the imaging axes of the displacements field and its gradient, respectively.}
...
The derivative of the data term is computed using explicit shape gradients (see \ref{app:shape_gradients}),
  that finally lead to obtain vertex-wise speeds of the gradient $\bar{s}_i$ as}
...
Using the expressions \eqref{eq:regseg-gradient_wshape} and 
  ...
  and introducing the analytical derivative of the regularization term, then
...
\revcomment[R\#3-C32]{As a rule of thumb, for a $\delta =$ 1.0 both $\boldsymbol{\alpha}$
...
The implementation details as well as other features (such as the sparse matrix approach
  ...
  ($\delta$, $\boldsymbol{\alpha}$, $\boldsymbol{\beta}$, the B-spline grid resolutions,
...
Then, \regseg{} is evaluated in a framework using undistorted \gls*{dmri} datasets,
  ...
  the subject.
Therefore, the deformation field is nonzero only in the \revcomment[R\#3-C.23]{\acrfull*{pe}} axis,
...
Distortion only occurs along the \gls*{pe} axis of the image, so we computed the
  ...
  \revcomment[R\#3-C.24]{$a_i$ is the area corresponding to each vertex $\vec{v}_i$},
