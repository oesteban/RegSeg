...
The workflow to generate the digital phantoms and assess
...
Using \emph{FreeSurfer} tools, the reference surfaces \gammaset{}
  are generated.
...
The warped surfaces are binarized to generate a distorted version of
  the model in two target resolutions: \isores[mm]{2.0} and \isores[mm]{1.0}.
...
To evaluate the error, we computed the Haussdorf distance (as implemented
 by \cite{commandeur_vtk_2011}) between the surfaces warped with the ground-truth
 and their corresponding contours estimated with \emph{regseg}.

\begin{figure*}
\includegraphics[width=\linewidth]{figures/figure03}
\caption{Evaluation workflow of phantoms.

}\label{fig:evphantoms}
\end{figure*}
