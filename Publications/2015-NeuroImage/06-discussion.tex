...
\revcomment[R\#1-C3]{%
\Regseg{} allows to avoid resampling and/or unwarping because the information necessary in the diffusion analysis is projected
  from the reference space.}
\revcomment[R\#1-C1]{%
Certain applications (like \gls*{tbss}) and methodologies (like building the connectivity matrix
  by clustering the tracks) may not perform on the native (distorted) diffusion space correctly,
  but they still need a mapping to the reference space.
