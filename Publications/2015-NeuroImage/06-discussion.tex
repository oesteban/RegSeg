...
\revcomment[R\#3-C6]{%
Therefore, we propose here to perform the reconstruction and tractography processes in the
  original (distorted) diffusion data.}
\revcomment[R\#1-C1]{%
Depending on the application (like \gls*{tbss}, or connectivity analyses that do not build the
  network matrix upon anatomical information mapped from a reference image), the obtained tracks
  may be geometrically unwarped using the estimation of the deformation found by \regseg{}.
Conversely, when the information necessary in the diffusion analysis is projected from the reference
  space, \regseg{} allows to avoid resampling and/or unwarping.}
\revcomment[R\#3-C6]{%
This methodological variation should be further investigated, to ensure which processing design
