...
def GenerateSurface( data, fname ):
    ...
    w = tvtk.PolyDataWriter(input=iso.output, file_name=os.path.join( model_path, fname ) )
    w.write()


GenerateSurface( data_csf.astype( np.float ), 'csf_prior.vtk')
GenerateSurface( data_filled.astype(np.float), 'wm_prior.vtk')
GenerateSurface( data_filled.astype(np.float) + data_gm.astype(np.float) , 'gm_prior.vtk')
